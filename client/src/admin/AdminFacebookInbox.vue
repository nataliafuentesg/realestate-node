...
async function loadConversations() {
  try {
    const res = await api.get('/facebook/conversations')
...
async function selectConversation(senderId) {
  ...
  try {
    const res = await api.get(`/facebook/conversations/${senderId}`)
...
async function sendReply() {
  ...
  sending.value = true
  try {
    await api.post(`/facebook/conversations/${selectedSenderId.value}/reply`, { body: replyText.value })
    replyText.value = ''
...
<template>
  <div class="flex h-[calc(100vh-6.5rem)] flex-col font-[family-name:var(--font-mp-body)] lg:h-[calc(100vh-6rem)]">
    <ConnectedAccountBadge platform="facebook" />
    <h1 class="font-[family-name:var(--font-mp-heading)] text-2xl font-medium text-mp-fg">📘 Facebook Messenger</h1>
    ...
    <div class="mt-4 flex min-h-0 flex-1 gap-6 lg:mt-6">
      ...
      <div
        ...
        :class="selectedSenderId ? 'hidden' : 'block'"
      >
